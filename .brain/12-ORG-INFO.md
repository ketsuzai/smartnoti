...
- **파일**: `src/pages/oper/operation-org-info.html` ✅ 완료
- **역할**: 현재 기관의 기본정보 조회 및 수정 (운영관리 컨텍스트 내)
- **권한**: `const canEdit = (userRole === 'super' || userRole === 'org_admin')` — teacher는 조회 전용("👁️ 조회 전용" 뱃지)
- **섹션 구성**: 프로필 헤더 / 기본정보 / 운영정보 / 위치정보 / 서비스 설정 / 담당자
- **뷰↔편집**: [수정] 버튼 클릭 시 인라인 편집 전환, [저장]/[취소] 제공
- **UI 포인트**: 섹션별 카드 분리, 뷰 모드에서는 텍스트, 편집 모드에서는 input/select
